 ...
 * 
 * <p>If the value {@link LoggerFilter::DENY} is returned, then the logs event is
 * dropped immediately without consulting with the remaining
 * filters. 
 * 
 * <p>If the value {@link LoggerFilter::NEUTRAL} is returned, then the next filter
 * in the chain is consulted. If there are no more filters in the
 * chain, then the logs event is logged. Thus, in the presence of no
 * filters, the default behaviour is to logs all logging events.
 * 
 * <p>If the value {@link LoggerFilter::ACCEPT} is returned, then the logs
...
abstract class LoggerFilter extends LoggerConfigurable {
	...
	/**
	 * The logs event must be logged immediately without consulting with
	 * the remaining filters, if any, in the chain.	 
	 */
	const ACCEPT = 1;
	
	/**
	 * This filter is neutral with respect to the logs event. The
	 * remaining filters, if any, should be consulted for a final decision.
	 */
	const NEUTRAL = 0;
	
	/**
	 * The logs event must be dropped immediately without consulting
